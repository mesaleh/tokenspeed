...
This is the A17-N8-R1-D0 composition micro-gate.  It deliberately stops at a
...
def kernel(
    ...
    for group in cutlass.range_constexpr(_GROUPS):
        ...
        dst[group, lane, 0] = regs[0].to(cutlass.Uint32)
        dst[group, lane, 1] = regs[1].to(cutlass.Uint32)
...
def host(
    ...
    kernel(tma_src, dst, POISON).launch(
        grid=(1, 1, 1),
...
def run(poison: int) -> tuple[torch.Tensor, torch.Tensor]:
    codes = _codes()
...
def verify() -> None:
    ...
    first, expected = run(0x00)
    second, expected_second = run(0x5A)
    ...
    print(
        ...
        "codes=16 padding_poison_independent=True",
        flush=True,
    )
