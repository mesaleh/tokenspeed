#!/usr/bin/env python3
"""Evaluate R26 on the sealed R19 three-role, all-layer holdout."""
...
EXPERIMENT_ID = "a17-n10-e0-r26-q1-20260818"
DECISION_ADVANCE = "ADVANCE_R26_ALL_LAYER_TO_S0"
DECISION_REJECT = "REJECT_R26_Q1"
...
class EvidenceError(RuntimeError):
    """Frozen inputs cannot support an R26-Q1 decision."""
...
def _load_r19(path: Path) -> ModuleType:
    ...
    try:
        module = _load_module(path, "a17_r26_q1_frozen_r19", EXPECTED_R19_SOURCE_SHA256)
    finally:
...
def _quality_metrics(
    *,
    ...
    if not torch.isfinite(score).all():
        raise EvidenceError("R26-Q1 candidate score is nonfinite")
...
def _evaluate_layer(
    *,
    ...
    high_metrics = r26._float8_diagnostics(
        ...
        label=f"R26-Q1 high RoPE {role}:{layer}",
    )
    surfaces = []
    for label, q_nope, q_pe, q_inc, query_position in r19._surface_specs(capture):
        ...
        qrot = r19.q0_math.e4m3fn(
            ...
            label=f"R26-Q1 rotated query {role}:{layer}:{label}",
        )
        ...
        q_high, q_residual, query_metrics = r26._query_rope_e4m3(
            q_pe, layer_id=layer, label=f"{role}:{label}"
        )
        candidate_score = r26._hierarchical_score(
            q_rot=qrot,
            raw=raw,
            q_high=q_high,
            high=stored_high[eligible].to(torch.float32),
        ...
        surface_pass = (
            ...
            and query_metrics["high"]["endpoint_count"] == 0
            and query_metrics["residual"]["endpoint_count"] == 0
        )
        surfaces.append(
            {
                "label": label,
...
def _validate_complete_roles(roles: list[dict[str, Any]]) -> int:
    try:
        ...
        raise EvidenceError("R26-Q1 role/cell structure is malformed") from error
    expected = {
        (role, layer, surface)
        for role in EXPECTED_ROLES
    ...
    if cells != expected:
        raise EvidenceError("R26-Q1 evaluated cell set differs")
    return len(cells)
...
def evaluate(args: argparse.Namespace) -> dict[str, Any]:
    if torch.get_num_threads() != 1:
        raise EvidenceError("R26-Q1 requires exactly one Torch thread")
    r26 = _load_module(
        args.r26_source, "a17_r26_q1_frozen_q0", EXPECTED_R26_SOURCE_SHA256
    )
    r19 = _load_r19(args.r19_source)
    r18 = r19._load_module(
        args.r18_source, "a17_r26_q1_frozen_r18", r19.R18_SOURCE_SHA256
    )
    packer = r19._load_module(
        args.packer_source, "a17_r26_q1_frozen_packer", r19.PACKER_SHA256
    )
    ...
    if bridge_seen != set(bridge):
        raise EvidenceError("R26-Q1 did not reproduce every available R19 bridge cell")
    surfaces = [
        surface
        for role in roles
        for layer in role["layers"]
