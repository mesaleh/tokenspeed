...
def require_zero_recovery(recovery: dict, execution_raw: bytes, cell: str) -> None:
    require(
...
def main() -> int:
    ...
    baseline_m128_seal, _ = load_json(args.baseline_m128_seal)
    baseline_dense, baseline_dense_raw = load_json(args.baseline_dense)
    baseline_dense_seal, _ = load_json(args.baseline_dense_seal)
    require_baseline(
        baseline_m128,
        baseline_m128_raw,
        baseline_m128_seal,
    ...
    for cell in SANITIZER_CELLS:
        require_zero_recovery(recoveries[cell][0], execution[cell][1], cell)
