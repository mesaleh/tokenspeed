...
def build_inputs(B, q_len, seq_k, seed=0):
    ...
    pages_per = ((pages_per + pages_per_tile - 1) // pages_per_tile) * pages_per_tile
    num_pages = pages_per * B
    kv_cache = (torch.randn(num_pages, PAGE, D, device=DEV, generator=g) * 0.3).to(FP8)
    block_tables = torch.arange(num_pages, device=DEV, dtype=torch.int32).view(
        B, pages_per
    )
...
def random_tree_ancestor(B, q_len, seed=1):
    ...
    for b in range(B):
        parent = [-1] + [
            int(torch.randint(0, i, (1,), generator=g)) for i in range(1, q_len)
        ]
...
def run_case(B, q_len, seq_k, fp8_tol, expected_unsupported_q, args):
    ...
    for name, anc in (
        ...
        # Exercise both the explicit ABI and the graph-safe device derivation.
        cmask_off = (
            torch.arange(B, dtype=torch.int32, device=DEV) * (q_len * seq_k)
            if name == "chain"
            else None
        )
        try:
        ...
        if not ok:
            return False
        if name == "random" and seq_k <= 2 * q_len:
            # Negative control: this shape deliberately gives tree columns
            # enough attention mass that ignoring custom_mask cannot pass the
            # same absolute tolerance as the real tree output.
            o_unmasked = call(
                q,
                kv,
                bt,
            ...
            unmasked_delta = (o_unmasked - o_ref).abs().max().item()
            load_bearing = unmasked_delta > fp8_tol
            print(
                "            negative-control unmasked vs tree-ref "
                f"Delta={unmasked_delta:.2e} "
                f"{'OK' if load_bearing else 'FAIL'}"
            )
            if not load_bearing:
                return False
...
def main():
    ...
    if args.cases is not None:
        ...
        # caused the OOB anc_bits read on the server. K=q cases make the negative
        # control load-bearing; the remaining cases cover dt=8 and bs=2.
        cases = [
            (1, 8, 8),
            (2, 8, 8),
            (1, 4, 256),
    ...
    for B, q_len, seq_k in cases:
        allpass = (
            run_case(B, q_len, seq_k, FP8_TOL, expected_unsupported_q, args) and allpass
        )
